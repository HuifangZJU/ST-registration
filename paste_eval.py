...
def apply_procrustes_transform(image, R, tX, tY):
    """
    Applies the computed Procrustes transformation (rotation + translation) to an image.

    Args:
        image: The moving image to be transformed.
        R: 2×2 rotation matrix.
        tX: Translation vector for X-coordinates.
        tY: Translation vector for Y-coordinates.
    ...
    # Compute full affine transformation matrix
    transformation_matrix = np.hstack([R, np.array([[tX[0]], [tX[1]]])])
...
image_id = ["0_0_image","0_1_image","0_2_image","0_3_image","1_0_image","1_1_image","1_2_image","1_3_image","2_0_image","2_1_image","2_2_image","2_3_image"]

adatas = {sample:sc.read_h5ad('../data/DLPFC/{0}_preprocessed.h5'.format(sample)) for sample in sample_list}


a = np.load("/home/huifang/workspace/code/registration/data/DLPFC/huifang/0_0_pca_out.npy")
print(a)
test = input()
...
for j in range(len(layer_groups)):
    ...
    aligned_slices, R, T = pst.stack_slices_pairwise(layer_groups[j], pis[j], output_params=True, matrix=True)
    paste_layer_groups.append(aligned_slices)
