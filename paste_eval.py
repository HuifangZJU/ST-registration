...
def overlay_images(image1, image2, alpha=0.5):
    """
    Overlays two RGB images using alpha blending.

    Args:
        image1: First image (numpy array).
        image2: Second image (numpy array).
        alpha: Blending weight for image1 (0.0 to 1.0).
    ...
    # Alpha blending: (alpha * image1) + ((1 - alpha) * image2)
...
def find_transformed_image_limit(image,R,T,pts):
    h, w = image.shape[:2]
    ...
    # plt.scatter(transformed_corners[:,0],transformed_corners[:,1],color='red')
    # plt.show()
...
def get_adjusted_affine_matrix(T,R,image_shift):
    # Adjust translation to shift the image into positive space
    # adjusted_T = T - np.array(image_shift)

    # Construct the affine transformation matrix
    # Step 1: Translate image so centroid aligns with the origin
    ...
    # # Step 3: Translate image back to its new position
    # translate_back = np.array([
    #     [1, 0, -adjusted_T[0]],  # Move back to new position
    #     [0, 1, -adjusted_T[1]]
    # ])

    # Step 4: Compute final transformation matrix
    # final_matrix = translate_back @ np.vstack([affine_matrix, [0, 0, 1]]) @ np.vstack([translate_to_origin, [0, 0, 1]])
...
def apply_affine_transformation(image1, R1, T1,pts1, image2,R2,T2,pts2):
    ...
    """
    x_min1, y_min1, x_max1, y_max1 = find_transformed_image_limit(image1,R1,T1,pts1)
    x_min2, y_min2, x_max2, y_max2 = find_transformed_image_limit(image2, R2, T2,pts2)
...
for id,pid in zip(sample_list,processed_id):
    ...
    # paste_coor = adatas[id].obsm['spatial']
    # s, T = find_scale_translation(paste_coor, info["coord"])
    # # Load moving image
    # img_moving = cv2.imread(adatas[id].image_path)  # Load your image
    # # Apply transformation, cropping, and point adjustment
    #
    # transformed_img, adjusted_fixed_pts = warp_image_resize_points(img_moving, info["coord"], paste_coor,output_size=(1024, 1024))
    # # print(transformed_img.shape)
    # # plt.imshow(transformed_img)
    # # plt.scatter(adjusted_fixed_pts[:,0],adjusted_fixed_pts[:,1])
    # # plt.show()
    # adatas[id].obsm['spatial'] = adjusted_fixed_pts
    # adatas[id].pasteimage= transformed_img
...
for j in range(len(layer_groups)):
    ...
    for i in range(len(slices)-1):
        ...
        a[0, 0].scatter(pts_slice1[:, 0], pts_slice1[:, 1], color="red", s=10,
                        label="Slice 1")
        ...
        a[0, 1].scatter(pts_slice2[:, 0], pts_slice2[:, 1], color="blue", s=10,
                        label="Slice 2")
        ...
        a[1,0].scatter(adjusted_warped_slice1[:, 0], adjusted_warped_slice1[:, 1], color="red", s=10,
        ...
        a[1,1].scatter(adjusted_warped_slice2[:, 0], adjusted_warped_slice2[:, 1], color="blue", s=10,
        ...
        a[0, 0].scatter(pts_slice1[:, 0], pts_slice1[:, 1], label="Slice 1", alpha=0.6)
        a[0, 0].scatter(pts_slice2[:, 0], pts_slice2[:, 1], label="Slice 2", alpha=0.6)
        ...
        a[0, 1].scatter(adjusted_warped_slice1[:, 0], adjusted_warped_slice1[:, 1], label="Warped Slice 1", alpha=0.6)
        a[0, 1].scatter(adjusted_warped_slice2[:, 0], adjusted_warped_slice2[:, 1], label="Warped Slice 2", alpha=0.6)
        a[0, 1].invert_yaxis()
